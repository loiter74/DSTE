...
class PriSTI(nn.Module):
    def __init__(self, config, inputdim=2, target_dim=3, is_itp=False):
        super().__init__()
        config["channels"] = 512
        ...
        if 1:
            ...
            self.itp_projection = Conv1d_with_init(inputdim, self.itp_channels, 1)

            self.itp_modeling = GuidanceConstruct(channels=self.itp_channels, nheads=4, target_dim=3,
                                            order=2, include_self=True, device=config["device"], proj_t=config['channels'])
            self.cond_projection = Conv1d_with_init(config["covariate_dim"], self.itp_channels, 1)
        ...
        self.residual_layers = nn.ModuleList(
            [
                NoiseProject(
                    side_dim=config["covariate_dim"],
                    channels=self.channels,
                    diffusion_embedding_dim=config["diffusion_embedding_dim"],
                    nheads=4,
                    target_dim=target_dim,
                    proj_t=config["channels"],
                    is_adp=True,
    ...
    def forward(self, x, side_context, pred_context, side_info, pred_target, A,  context_missing, diffusion_step, is_train):
        itp_x = x
        if self.is_itp:
            x = torch.cat([x, itp_x], dim=2)
        B, K, inputdim, L = x.shape
        ...
        if 1:
            itp_x = itp_x.reshape(B, inputdim, K * L)
...
class NoiseProject(nn.Module):
    ...
    def forward(self, x, side_info, diffusion_emb, itp_info, support):
        ...
        _, _, side_dim,  _ = side_info.shape
...
class AdaptiveGCN(nn.Module):
    def __init__(self, channels, order=2, include_self=False, device=None, is_adp=True, adj_file=None):
        super().__init__()
        self.order = order
        ...
        self.mlp = nn.Conv2d(1536, c_out, kernel_size=1) # temp
...
class SpatialLearning(nn.Module):
    def __init__(self, channels, nheads, target_dim, order, include_self, device, is_adp, adj_file, proj_t, is_cross):
        ...
        self.feature_layer = SpaDependLearning(channels, nheads=nheads, order=order, target_dim=target_dim,
                                               include_self=include_self, device=device,
...
class SpaDependLearning(nn.Module):
    def __init__(self, channels, nheads, target_dim, order, include_self, device, proj_t,
                 is_cross=True):
        super().__init__()
        self.is_cross = is_cross


        self.GCN = AdaptiveGCN(channels, order=order, include_self=include_self, device=device,
                               )
...
class GuidanceConstruct(nn.Module):
    ...
    def forward(self, y, base_shape, support):
        ...
        y_in2 =  y_attn_s + y_attn_t #+y_local
        y = F.relu(self.ff_linear1(y_in2.permute(0, 2, 1)))
        y = self.ff_linear2(y).permute(0, 2, 1)
        y = y + y_in2

        y = self.norm2(y).reshape(B, K, channel, L)
        return y
