...
class LatentLayer(nn.Module):
    ...
    def forward(self, x):
        b, num_m, _, t = x.shape
        x = x.contiguous().view([b * num_m, -1, t])
...
class InferenceModel(nn.Module):
    def __init__(self, tcn_channels, latent_channels, num_hidden_layers):
        ...
        self.layers_c = nn.ModuleList([LatentLayer(tcn_channels[i], 0, latent_channels[i], latent_channels[i], num_hidden_layers) for i in range(len(tcn_channels))])
        self.latent_layer = nn.ModuleList([nn.Conv1d(latent_channels[i], tcn_channels[i-1], 1) for i in range(1, len(tcn_channels))])
    ...
    def forward(self, d_c, d_t, adj, missing_index_context, training=True):
        ...
        #z = [dists[-1].mean] if training else [dists[-1].mean]
