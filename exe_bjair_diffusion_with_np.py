...
if __name__ == "__main__":
    ...
    parser.add_argument("--np_model_path", type=str, default="save/np/BJAir2017/PM25/model_202505182244.pth",
                        help="Path to the pre-trained NP model")
    ...
    parser.add_argument("--test_phase", type=str, default="in", help="Test phase: 'in', 'out', 'LD_out', or 'all'")
    ...
    # 使用参数指定的预训练神经过程模型路径
    np_pretrained = opt.np_model_path
    ...
    if opt.test_phase in ["in", "all"]:
        ...
        print("CRPS (in-distribution): ", CRPS)
    ...
    if opt.test_phase in ["out", "all"]:
        ...
        for i in range(len(mask)):
            mask[i] = (1 - mask[i]) * (1 - mask_imputes[i].unsqueeze(2).to(opt.device))
        CRPS = _quantile_CRPS_with_missing(pred_samples, label, mask)
        print("CRPS (out-of-distribution): ", CRPS)

    if opt.test_phase in ["LD_out", "all"]:
        print("Testing on LD out-of-distribution data...")
        ...
        for i in range(len(mask)):
            mask[i] = (1 - mask[i]) * (1 - mask_imputes[i].unsqueeze(2).to(opt.device))
        CRPS = _quantile_CRPS_with_missing(pred_samples, label, mask)
        print("CRPS (LD out-of-distribution): ", CRPS)
